...
WARRIOR3D._viewmodel = function (player) {
  ...
  // bow (hidden until aim/shoot) — compact, held lower-centre
  const bg = new THREE.Group();
  const bow = new THREE.Mesh(new THREE.TorusGeometry(0.19, 0.014, 8, 24, Math.PI * 1.2), new THREE.MeshStandardMaterial({ color: 0x5a3a1e, roughness: 0.8 }));
  bow.rotation.z = Math.PI * 0.4; bg.add(bow);
  const strG = new THREE.Group(); bg.add(strG);
  const str = new THREE.Mesh(new THREE.CylinderGeometry(0.0035, 0.0035, 0.36, 4), new THREE.MeshBasicMaterial({ color: 0xddddcc })); strG.add(str);
  const nock = new THREE.Mesh(new THREE.CylinderGeometry(0.007, 0.007, 0.5, 5), steel); nock.rotation.x = Math.PI / 2; nock.position.z = -0.2; strG.add(nock);
  bg.position.set(0.06, -0.34, -0.82); bg.visible = false; g.add(bg);

  g.userData = { katana: kg, bow: bg, bowString: strG, arm, rest: { pos: kg.position.clone(), rot: kg.rotation.clone() } };
  return g;
};
...
WARRIOR3D._updateViewmodel = function (E, dt) {
  ...
  if (showBow) {
    if (E.vmClip === "aim") { V.bowString.position.z = THREE.MathUtils.lerp(V.bowString.position.z, 0.16, dt * 10); }
    else { const t = Math.min(1, E.vmT / 0.2); V.bowString.position.z = THREE.MathUtils.lerp(0.16, 0, t); if (t >= 1) WARRIOR3D._vm(E, E.aiming ? "aim" : "idle"); }
    return;
  }

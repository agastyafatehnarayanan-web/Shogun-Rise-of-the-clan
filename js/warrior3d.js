...
WARRIOR3D._nextFoe = function (E) {
  ...
  mesh.position.set(E.px + Math.sin(ang) * 7, 0, E.pz - 9); mesh.rotation.y = Math.PI; E.scene.add(mesh);
...
WARRIOR3D._updateCamera = function (E, dt) {
  ...
  E.yaw += (E.yawTarget - E.yaw) * Math.min(1, dt * 10); E.pitch += (E.pitchTarget - E.pitch) * Math.min(1, dt * 10);
  E.yawTarget *= (1 - Math.min(1, dt * 1.4)); E.pitchTarget *= (1 - Math.min(1, dt * 1.4));
...
WARRIOR3D._bindControls = function (E) {
  ...
  const moveTo = (x, y, id) => { if (E.joy.active && E.joy.id === id) doMove(x, y); else if (look.active && look.id === id) { E.yawTarget = THREE.MathUtils.clamp(E.yawTarget + (x - look.lx) * -0.006, -1.5, 1.5); E.pitchTarget = THREE.MathUtils.clamp(E.pitchTarget + (y - look.ly) * -0.005, -0.5, 0.5); look.lx = x; look.ly = y; } };
